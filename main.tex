...
The vector $\mathbf{h_\ell}$ can be computed easily from $\mathbf{h}$ in equation \eqref{eq:h}.
...
\end{equation}
and can be computed in $O((n-r)2^{n-r})$ cost given $\mathbf{h'_\ell}$ using FFT algorithms. The vector $\mathbf{h'_\ell}$ can be computed easily from $\mathbf{h}$ in equation \eqref{eq:h}.
